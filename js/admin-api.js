/**
 * A+ Auto Centre - Admin Portal JavaScript (API Version)
 * Handles vehicle management with backend API
 */
...
// Load vehicles from API on page load
...
document.getElementById('vehicleForm').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (currentEditId) {
            ...
            response = await fetch(`${API_URL}/vehicles/${currentEditId}`, {
                method: 'PUT',
...
async function loadVehiclesFromAPI() {
    try {
        ...
        showAlert('Error loading vehicles: ' + error.message, 'danger');
        vehicles = [];
    }
}
...
async function deleteVehicle(id) {
    ...
    try {
        ...
        showAlert('Error deleting vehicle: ' + error.message, 'danger');
    }
}
...
async function exportWebsiteData() {
    try {
        const response = await fetch(`${API_URL}/export/vehicle-data`);
        if (!response.ok) {
...
async function exportNLClassifieds() {
    if (vehicles.length === 0) {
        showAlert('No vehicles to export!', 'danger');
        return;
    }
